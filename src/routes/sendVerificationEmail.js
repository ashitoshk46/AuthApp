...
router.post('/sendVerification', resendLimiter, async (req, res) => {
  ...
  try {
    ...
    const attemptResult = await pool.query(
      `SELECT id, verification_tries, next_verification_date
    ...
    if (attemptResult.rows.length > 0) {
      ...
      if (verificationTries === 0) {
        if (nextVerificationDate && new Date() < nextVerificationDate) {
          ...
          return res
            .status(429)
            .json({ error: `Try again after ${nextVerificationDate}` });
        }
      ...
      await pool.query(
        `INSERT INTO verification_attempts
           (user_id, verification_tries, next_verification_date)
         VALUES ($1, $2, $3)`,
        [userId, verificationTries, nextVerificationDate]
      );
    }

    // Use EmailService resend flow (handles token, email, and audit log)
    await emailService.sendVerificationEmail(userId, primary_email, {
      ipAddress: req.ip,
      resend: true,
    });
